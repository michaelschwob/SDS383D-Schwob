...
for(k in 1:length(t1)){
    for(j in 1:length(b)){
        ...
        #     Sig.cc <- matern.func(x[i], b[j], t1[k], t2) # should just be 1 + t2 # 1x1
        #     Sig.cc <- 1
        #     Sig.oo <- matern.func(x[-i], b[j], t1[k], t2) # (n-1)x(n-1)
        #     Sig.oc <- as.matrix(matern.vec(x[-i], x[i], b[j], t1[k], t2)) # (n-1)x1
        ...
        Sig.cc <- Sig.oo <- matern.func(x, b[j], t1[k], t2) # these are equal since the x's are equal
        Sig.oc <- matern.func(x, b[j], t1[k], t2)

        post.mean <- t(Sig.oc)%*%solve(Sig.oo)%*%Sig.oc

        Sig.bar <- Sig.cc - t(Sig.oc)%*%solve(Sig.oo)%*%Sig.oc

        lb <- post.mean - 1.96*sqrt(diag(Sig.bar))
        ub <- post.mean + 1.96*sqrt(diag(Sig.bar))
